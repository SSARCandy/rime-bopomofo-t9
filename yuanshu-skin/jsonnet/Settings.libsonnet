...
#======================================
{
  # 主键盘布局（此精簡版僅保留 t9zhuyin：T9 注音九宮格，bopomofo_t9 方案專用；
  # 其餘上游佈局的框架檔已移除，請勿改動此值）
  keyboardLayout: 't9zhuyin',


  # 数字键盘布局（此精簡版僅保留 t9：T9 數字九宮格，請勿改動此值）
  numericLayout: 't9',


  # 符号键盘（此精簡版固定使用元书自带的符号键盘，請勿改動此值）
  ...
  #
  # 将上述代号填入下面的数组即可
  # （按鈕數 ≤ toolbarSlideButtonsMaxCount 時直接並排顯示，不滑動）
  toolbarSlideButtons: [ 9, 3 ],  # 表情、剪貼簿

  # 是否顯示工具列最左側的「面板」按鈕（浮動面板入口：皮膚/微調/檔案等）
  # false 隱藏；需要時可從元書 App 內操作相同功能
